                ...
                <div class="p-6 bg-white shadow-lg border-b border-gray-200">
                    <div class="flex justify-between items-center mb-6">
                        <div class="flex items-center space-x-4">
                            <h1 class="text-lg md:text-2xl font-bold text-gray-800">Archived Tests</h1>
                        </div>
                    </div>
                    <div class="rounded-lg shadow-inner bg-gray-50">
                        <ul class="divide-y divide-gray-200">
                            @forelse ($archivedTests as $test)
                                <li class="p-3 sm:p-4 hover:bg-gray-100 transition duration-150 ease-in-out rounded-lg">
                                    <div class="flex justify-between items-center">
                                        <div class="flex-grow">
                                            <h3 class="text-base md:text-lg font-semibold text-gray-700">{{ $test->title }}</h3>
                                            <p class="text-sm md:text-base text-gray-600 mt-2">{{ Str::limit($test->description, 100) }}</p>
                                            <div class="flex items-center mt-4 text-xs md:text-sm text-gray-500">
