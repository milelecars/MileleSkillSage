                            ...
                            <div x-show="open" x-cloak @click.away="open = false" class="absolute right-0 mt-2 w-72 bg-white border border-gray-200 shadow-lg rounded-lg p-4 z-50">
                                <form method="GET" action="{{ route('admin.manage-candidates') }}" class="space-y-3">
                                    ...
                                    <div class="flex justify-between items-center pt-5">
                                        <a href="{{ route('admin.export-candidates', request()->only(['search', 'test_filter', 'name', 'email', 'role', 'department'])) }}"
                                ...
                                <tbody class="bg-white divide-y divide-gray-200 text-center">
                                    @forelse($candidates as $candidate)
                                        <tr class="relative items-center justify-center" 
                                            ...
                                            <td class="py-4 h-full text-xs md:text-sm">{{ $candidate['test_title'] }}</td>

                                            <td class="py-4 h-full text-xs md:text-sm">{{ $candidate['role'] ?? "-" }}</td>

                                            <td class="py-4 h-full text-xs md:text-sm">{{ $candidate['department'] ?? "-" }}</td>
